...
describe("lookupGuestAppointment", () => {
  it("returns the appointment when code and phone match", () => {
    const result = lookupGuestAppointment("LN-1002", "0977445566");
    expect(result).not.toBeNull();
    expect(result?.code).toBe("LN-1002");
  });
  ...
  it("returns null when the phone does not match the code", () => {
    const result = lookupGuestAppointment("LN-1002", "0909111222");
    expect(result).toBeNull();
  });

  it("returns null for an unknown code", () => {
    const result = lookupGuestAppointment("LN-9999", "0977445566");
    expect(result).toBeNull();
  });
});
